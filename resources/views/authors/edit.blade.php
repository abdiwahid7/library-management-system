            ...
            <form method="POST" action="{{ route('authors.update', $author->id) }}">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="biography" id="biography" rows="4"
                        class="shadow appearance-none border border-green-200 rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-green-400 @error('biography') border-red-500 @enderror">{{ old('biography', $author->biography) }}</textarea>
                    @error('biography')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
